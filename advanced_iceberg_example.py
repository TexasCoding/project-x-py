...
class AdvancedIcebergDetector:
    ...
    def __init__(self):
        ...
        self.price_level_history: Dict[float, Dict] = defaultdict(self._create_history_dict)
            ...
            if history['volume_history']:
                ...
                if prev_volume > 0 and volume > prev_volume * 1.2:  # 20% increase
                    if history['last_refresh_time']:
                        refresh_interval = (timestamp - history['last_refresh_time']).total_seconds()
    ...
    def _build_statistical_models(self):
        ...
        for price, history in self.price_level_history.items():
            ...
            if len(refresh_intervals) >= 2:
                ...
                refresh_regularity = 1.0 / (1.0 + interval_std / interval_mean) if interval_mean > 0 else 0
            else:
    ...
    def _identify_iceberg_patterns(self) -> List[Dict]:
        ...
        for price, model in self.volume_models.items():
            ...
            indicators = {
                ...
                'volume_significance': min(1.0, model['volume_mean'] / 1000),  # Normalized
            ...
            if composite_score >= 0.7:  # High threshold for institutional use
                ...
                estimated_hidden_size = self._estimate_hidden_size_advanced(price, model, composite_score)
                ...
                iceberg_candidate = {
                    ...
                    'total_estimated_size': int(model['volume_mean'] + estimated_hidden_size),
                    'refresh_count': len(history['refresh_intervals']),
                    'avg_refresh_interval': round(mean(history['refresh_intervals']), 2) if history['refresh_intervals'] else 0,
                    'volume_consistency_score': round(indicators['volume_consistency'], 3),
                    'refresh_regularity_score': round(indicators['refresh_regularity'], 3),
                    'detection_method': 'institutional_multi_factor_analysis',
                    'indicators': indicators,
                    'statistical_significance': self._calculate_statistical_significance(model)
                }
    ...
    def _estimate_hidden_size_advanced(self, price: float, model: Dict, confidence: float) -> int:
    ...
    def _calculate_statistical_significance(self, model: Dict) -> float:
        ...
        sample_factor = min(1.0, model['sample_size'] / 30)  # 30+ samples for high confidence
    ...
    def _calculate_detection_metrics(self, icebergs: List[Dict]) -> Dict:
        ...
        return {
            ...
            'high_confidence_count': sum(1 for i in icebergs if i['confidence_score'] >= 0.8),
            'total_estimated_hidden_volume': sum(i['estimated_hidden_size'] for i in icebergs),
            'avg_estimated_size_ratio': round(mean([
                i['estimated_hidden_size'] / i['current_visible_volume'] 
                for i in icebergs if i['current_visible_volume'] > 0
            ]), 2) if icebergs else 0
        }

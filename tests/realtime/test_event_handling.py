...
class MockEventHandler(EventHandlingMixin):
    ...
    def __init__(self):
        ...
        self.callbacks = defaultdict(
            list
        )  # Must be defaultdict like real implementation
...
class TestEventCallbackRegistration:
    ...
    async def test_register_multiple_callbacks(self, event_handler):
        ...
        assert all(
            cb in event_handler.callbacks["test_event"]
    ...
    async def test_unregister_nonexistent_callback(self, event_handler):
        ...
        assert (
            "test_event" not in event_handler.callbacks
            or len(event_handler.callbacks["test_event"]) == 0
        )
...
class TestBatchedEventHandling:
    ...
    @pytest.mark.asyncio
    async def test_enable_batching_quote_is_processed_without_typeerror(
        self, event_handler
    ):
        """enable_batching() must process quotes without awaiting a sync forwarder."""
        received: list[dict] = []
        ...
        quote = ["MNQ", {"bid": 18500.0, "ask": 18501.0}]
        await event_handler._batched_handler._process_quote_batch([quote])
        await asyncio.sleep(0.05)

        assert received, "quote should be processed when batching is enabled"
        assert received[0]["contract_id"] == "MNQ"
        assert received[0]["data"]["bid"] == 18500.0
    ...
    async def test_callback_timeout_handling(self, event_handler):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                event_handler._trigger_callbacks("test_event", {}), timeout=0.1
            )
